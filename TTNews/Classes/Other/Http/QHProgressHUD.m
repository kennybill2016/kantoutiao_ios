...
#define QHMainThreadAssert() NSAssert([NSThread isMainThread], @"QHProgressHUD needs to be accessed on the main thread.");

CGFloat const QHProgressMaxOffset = 1000000.f;

static const CGFloat QHDefaultPadding = 4.f;
static const CGFloat QHDefaultLabelFontSize = 16.f;
static const CGFloat QHDefaultDetailsLabelFontSize = 12.f;


@interface QHProgressHUD () {
    // Deprecated
...
@interface QHProgressHUDRoundedButton : UIButton
@end


@implementation QHProgressHUD

#pragma mark - Class methods

+ (instancetype)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
    QHProgressHUD *hud = [[self alloc] initWithView:view];
...
+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated {
    QHProgressHUD *hud = [self HUDForView:view];
    if (hud != nil) {
        hud.removeFromSuperViewOnHide = YES;
...
+ (QHProgressHUD *)HUDForView:(UIView *)view {
    NSEnumerator *subviewsEnum = [view.subviews reverseObjectEnumerator];
    for (UIView *subview in subviewsEnum) {
        if ([subview isKindOfClass:self]) {
            return (QHProgressHUD *)subview;
        }
    }
    return nil;
}
...
- (void)commonInit {
    ...
    _animationType = QHProgressHUDAnimationFade;
    _mode = QHProgressHUDModeIndeterminate;
    _margin = 20.0f;
    _opacity = 1.f;
...
- (void)animateIn:(BOOL)animatingIn withType:(QHProgressHUDAnimation)type completion:(void(^)(BOOL finished))completion {
    // Automatically determine the correct zoom animation type
    if (type == QHProgressHUDAnimationZoom) {
        type = animatingIn ? QHProgressHUDAnimationZoomIn : QHProgressHUDAnimationZoomOut;
    }
    ...
    if (animatingIn && bezelView.alpha == 0.f && type == QHProgressHUDAnimationZoomIn) {
        bezelView.transform = small;
    } else if (animatingIn && bezelView.alpha == 0.f && type == QHProgressHUDAnimationZoomOut) {
    ...
    dispatch_block_t animations = ^{
        ...
        } else if (!animatingIn && type == QHProgressHUDAnimationZoomIn) {
            bezelView.transform = large;
        } else if (!animatingIn && type == QHProgressHUDAnimationZoomOut) {
            bezelView.transform = small;
        }
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
...
- (void)done {
    ...
    QHProgressHUDCompletionBlock completionBlock = self.completionBlock;
    if (completionBlock) {
        completionBlock();
    }
    id<QHProgressHUDDelegate> delegate = self.delegate;
...
- (void)setupViews {
    ...
    QHBackgroundView *backgroundView = [[QHBackgroundView alloc] initWithFrame:self.bounds];
    backgroundView.style = QHProgressHUDBackgroundStyleSolidColor;
    ...
    QHBackgroundView *bezelView = [QHBackgroundView new];
    ...
    label.font = [UIFont boldSystemFontOfSize:QHDefaultLabelFontSize];
    label.opaque = NO;
    ...
    detailsLabel.font = [UIFont boldSystemFontOfSize:QHDefaultDetailsLabelFontSize];
    detailsLabel.opaque = NO;
    detailsLabel.backgroundColor = [UIColor clearColor];
    _detailsLabel = detailsLabel;

    UIButton *button = [QHProgressHUDRoundedButton buttonWithType:UIButtonTypeCustom];
    button.titleLabel.textAlignment = NSTextAlignmentCenter;
    button.titleLabel.font = [UIFont boldSystemFontOfSize:QHDefaultDetailsLabelFontSize];
...
- (void)updateIndicators {
    ...
    BOOL isRoundIndicator = [indicator isKindOfClass:[QHRoundProgressView class]];

    QHProgressHUDMode mode = self.mode;
    if (mode == QHProgressHUDModeIndeterminate) {
    ...
    else if (mode == QHProgressHUDModeDeterminateHorizontalBar) {
        // Update to bar determinate indicator
        [indicator removeFromSuperview];
        indicator = [[QHBarProgressView alloc] init];
        [self.bezelView addSubview:indicator];
    }
    else if (mode == QHProgressHUDModeDeterminate || mode == QHProgressHUDModeAnnularDeterminate) {
        if (!isRoundIndicator) {
            // Update to determinante indicator
            [indicator removeFromSuperview];
            indicator = [[QHRoundProgressView alloc] init];
            [self.bezelView addSubview:indicator];
        }
        if (mode == QHProgressHUDModeAnnularDeterminate) {
            [(QHRoundProgressView *)indicator setAnnular:YES];
        }
    } 
    else if (mode == QHProgressHUDModeCustomView && self.customView != indicator) {
        // Update custom view indicator
        [indicator removeFromSuperview];
        indicator = self.customView;
        [self.bezelView addSubview:indicator];
    }
    else if (mode == QHProgressHUDModeText) {
        [indicator removeFromSuperview];
        indicator = nil;
    }
...
#pragma clang diagnostic pop
    ...
    if ([indicator isKindOfClass:[UIActivityIndicatorView class]]) {
        ...
        appearance = [UIActivityIndicatorView appearanceWhenContainedIn:[QHProgressHUD class], nil];
#else
        // For iOS 9+
        appearance = [UIActivityIndicatorView appearanceWhenContainedInInstancesOfClasses:@[[QHProgressHUD class]]];
#endif
        
        if (appearance.color == nil) {
            ((UIActivityIndicatorView *)indicator).color = color;
        }
    } else if ([indicator isKindOfClass:[QHRoundProgressView class]]) {
        QHRoundProgressView *appearance = nil;
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [QHRoundProgressView appearanceWhenContainedIn:[QHProgressHUD class], nil];
#else
        appearance = [QHRoundProgressView appearanceWhenContainedInInstancesOfClasses:@[[QHProgressHUD class]]];
#endif
        if (appearance.progressTintColor == nil) {
            ((QHRoundProgressView *)indicator).progressTintColor = color;
        }
        if (appearance.backgroundTintColor == nil) {
            ((QHRoundProgressView *)indicator).backgroundTintColor = [color colorWithAlphaComponent:0.1];
        }
    } else if ([indicator isKindOfClass:[QHBarProgressView class]]) {
        QHBarProgressView *appearance = nil;
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [QHBarProgressView appearanceWhenContainedIn:[QHProgressHUD class], nil];
#else
        appearance = [QHBarProgressView appearanceWhenContainedInInstancesOfClasses:@[[QHProgressHUD class]]];
#endif
        if (appearance.progressColor == nil) {
            ((QHBarProgressView *)indicator).progressColor = color;
        }
        if (appearance.lineColor == nil) {
            ((QHBarProgressView *)indicator).lineColor = color;
        }
    } else {
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 70000 || TARGET_OS_TV
...
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 70000 || TARGET_OS_TV
    QHBackgroundView *bezelView = self.bezelView;
...
- (void)updatePaddingConstraints {
    ...
    [self.paddingConstraints enumerateObjectsUsingBlock:^(NSLayoutConstraint *padding, NSUInteger idx, BOOL *stop) {
        ...
        padding.constant = (firstVisible && (secondVisible || hasVisibleAncestors)) ? QHDefaultPadding : 0.f;
...
- (void)setMode:(QHProgressHUDMode)mode {
    if (mode != _mode) {
        _mode = mode;
        [self updateIndicators];
    }
...
- (void)setCustomView:(UIView *)customView {
    if (customView != _customView) {
        ...
        if (self.mode == QHProgressHUDModeCustomView) {
            [self updateIndicators];
        }
    }
}
...
@implementation QHRoundProgressView
...
@implementation QHBarProgressView
...
@implementation QHBackgroundView
...
- (instancetype)initWithFrame:(CGRect)frame {
    if ((self = [super initWithFrame:frame])) {
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_7_0) {
            _style = QHProgressHUDBackgroundStyleBlur;
            ...
            _style = QHProgressHUDBackgroundStyleSolidColor;
...
- (void)setStyle:(QHProgressHUDBackgroundStyle)style {
    if (style == QHProgressHUDBackgroundStyleBlur && kCFCoreFoundationVersionNumber < kCFCoreFoundationVersionNumber_iOS_7_0) {
        style = QHProgressHUDBackgroundStyleSolidColor;
    }
    if (_style != style) {
        _style = style;
        [self updateForBackgroundStyle];
...
- (void)updateForBackgroundStyle {
    QHProgressHUDBackgroundStyle style = self.style;
    if (style == QHProgressHUDBackgroundStyleBlur) {
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 80000 || TARGET_OS_TV
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_8_0) {
...
- (void)updateViewsForColor:(UIColor *)color {
    if (self.style == QHProgressHUDBackgroundStyleBlur) {
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_8_0) {
            self.backgroundColor = self.color;
        } else {
#if !TARGET_OS_TV
...
@implementation QHProgressHUD (Deprecated)

#pragma mark - Class

+ (NSUInteger)hideAllHUDsForView:(UIView *)view animated:(BOOL)animated {
    NSArray *huds = [QHProgressHUD allHUDsForView:view];
    for (QHProgressHUD *hud in huds) {
        hud.removeFromSuperViewOnHide = YES;
        [hud hideAnimated:animated];
    }
    return [huds count];
...
- (void)showAnimated:(BOOL)animated whileExecutingBlock:(dispatch_block_t)block onQueue:(dispatch_queue_t)queue completionBlock:(nullable QHProgressHUDCompletionBlock)completion {
...
- (BOOL)dimBackground {
    QHBackgroundView *backgroundView = self.backgroundView;
    UIColor *dimmedColor =  [UIColor colorWithWhite:0.f alpha:.2f];
    return backgroundView.style == QHProgressHUDBackgroundStyleSolidColor && [backgroundView.color isEqual:dimmedColor];
}

- (void)setDimBackground:(BOOL)dimBackground {
    QHMainThreadAssert();
    self.backgroundView.style = QHProgressHUDBackgroundStyleSolidColor;
...
@implementation QHProgressHUDRoundedButton
